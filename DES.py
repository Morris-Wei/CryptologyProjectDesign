from AllConvert import hextoasciiStr, asciistrtohex
...
class des():
    ...
    def run(self, key, text, action=ENCRYPT, padding=False):
        ...
        if padding and action == DECRYPT:
            final_res = self.removePadding(final_res)
            final_res = asciistrtohex(final_res)
            return final_res  # Remove the padding if decrypt and padding is true
        else:
            final_res = asciistrtohex(final_res)
    ...
    def encrypt(self, key, text, padding=False):
        key = hextoasciiStr(key)
        text = hextoasciiStr(text)
...
if __name__ == '__main__':
    key = "3243f6a8885a308d"
    text = "dbaeefbd59bff9c3"
    d = des()
